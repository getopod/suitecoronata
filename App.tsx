...
const ICON_SYNONYMS: Record<string, string> = {
      // Explicit mappings for all provided curse icons
...
export default function SolitaireEngine({ 
  ...
  const [selectedMode, setSelectedMode] = useState('coronata');
   const [glossaryTab, setGlossaryTab] = useState<'blessings'|'exploits'|'curses'|'patterns'>('curses');
  ...
  const resolveWander = (choice: WanderChoice) => {
    ...
    // OLD-STYLE EFFECTS (DEPRECATED)
    if (choice.effects) {
      // Apply old-style effects
      const applied = applyWanderEffects(gameState, choice.effects);
      setGameState(prev => ({ ...prev, ...applied, wanderState: 'result', wanderResultText: choice.result }));
    }
  };
  ...
  const finishWander = () => {
    ...
    Object.keys(newCharges).forEach(key => newCharges[key] = 0);

    if (nextIdx < runPlan.length) {
        // Check if blessing select should be shown before this encounter
        if (nextIdx > 0 && nextIdx % 3 === 0 && !preEncounterBlessing) {
           ...
           setGameState(prev => ({ ...prev, wanderState: 'none', wanderRound: 0, charges: newCharges }));
           return;
        }

        let nextEncounter = runPlan[nextIdx];
        // After blessing selection, reset preEncounterBlessing so it doesn't block future encounters
        if (preEncounterBlessing) setPreEncounterBlessing(false);
  ...
  const attemptMove = (cardIds: string[], sourcePileId: string, targetPileId: string, sourceCardIndex?: number): boolean => {
   ...
   const completeLevel = () => {
      ...
      // After shop is closed, advance to next encounter and reset the board
      // This requires a callback or effect to detect when the shop is closed
      // We'll use a useEffect to watch for the shop drawer closing after being open
   };
// --- PATCH: Advance to next encounter after shop closes ---
   React.useEffect(() => {
         // Only advance if the shop drawer was just closed after being locked (encounter complete)
         // Track previous drawer to detect shop close
         const prevDrawerRef = React.useRef<string | null>(null);
         React.useEffect(() => {
         ...
         React.useEffect(() => {
            if (
               prevDrawerRef.current === 'shop' && // shop was open
               activeDrawer === null && // now closed
               nonClosableDrawer === null && // lock released
         ...
         }, [activeDrawer, nonClosableDrawer, showLevelComplete, currentView, gameState.isLevelComplete]);
  ...
  const buyKeyWithTaxLoophole = () => {
    if (!activeEffects.includes('metro_card')) return;
  ...
  if (CLASSIC_GAMES[selectedMode]) {
    ...
    zoomScale = Math.min(1, (screenWidth - 2) / requiredWidth); // -2 for some breathing room
  } else {
                     ...
                     <div className="max-w-md mx-auto mt-2">
                        {activeDrawer === 'pause' ? (
                           <div className="grid grid-cols-5 gap-1">
                              ...
                              <button className="p-1 bg-slate-700 rounded flex flex-col items-center gap-0.5 text-slate-300 hover:bg-slate-600" onClick={() => setActiveDrawer('settings')}><img src="/icons/settings.png" alt="" className="w-8 h-8" /><span className="text-[7px]">Settings</span></button>
                           </div>
                        ) : activeDrawer === 'shop' ? (
                           ...
                           <div className="flex flex-col gap-1">
                              <button className="bg-slate-500 text-white px-2 py-1 rounded text-sm font-medium" onClick={() => setGameState(p => ({ ...p, score: p.currentScoreGoal }))}>Next</button>
                                 ...
                                 {blessingChoices.map(item => {
                                    ...
                                    <div key={item.id} className={`p-2 rounded border ${rarityColors.border} ${rarityColors.bg} flex items-center gap-3 cursor-pointer hover:brightness-110`} onClick={() => {
                                         ...
                                         if (preEncounterBlessing) {
                                            // Pre-encounter blessing: increment runIndex and start the encounter directly
                                            ...
                                            // Now start the encounter for the new runIndex
                                            setTimeout(() => {
                                               startNextEncounter();
                                            }, 0);
            ...
            {!CLASSIC_GAMES[selectedMode] ? (
               <>
                  ...
                  <div className="flex w-full gap-1">
                     <button onClick={() => toggleDrawer('pause')} className={`w-8 h-8 flex items-center justify-center bg-slate-800 hover:bg-slate-700 rounded text-slate-400 border border-slate-700 ${activeDrawer === 'pause' ? 'bg-slate-700' : ''}`}><img src="/icons/pause.png" alt="Pause" className="w-8 h-8" /></button>
                     <button type="button" onClick={undoLastMove} aria-label="Undo" className="shrink-0 w-8 h-8 flex items-center justify-center bg-slate-800 hover:bg-slate-700 rounded text-slate-400 border border-slate-700">
                        <img src="/icons/back.png" alt="Undo" className="w-8 h-8" />
                     </button>
                     <button type="button" className="shrink-0 w-8 h-8 flex items-center justify-center bg-transparent hover:bg-slate-700 rounded relative">
                        <ResponsiveIcon name="coin" fallbackType="exploit" size={32} className="w-8 h-8" alt="coins" />
                     ...
                     {(['exploit', 'blessing'] as const).map((type) => {
                        ...
                        return (
                           <button key={type} onClick={() => toggleDrawer(type as any)}
                              className={`w-8 h-8 flex items-center justify-center rounded text-[10px] font-bold border flex-col gap-0.5
                              ${activeDrawer === type ? 'bg-slate-700 text-white border-slate-600' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
                              <img src={categoryIcons[type]} alt="" className="w-8 h-8" />
                           </button>
                        );
                     })}

                     <button onClick={() => setPatternDrawer(true)}
                        className={`w-8 h-8 flex items-center justify-center rounded text-[10px] font-bold border flex-col gap-0.5
                        ${patternDrawer ? 'bg-blue-700 text-white border-blue-600' : 'bg-slate-800 text-blue-300 border-slate-700'}`}
                        aria-label="Show Patterns">
                        <span className="text-2xl">𝌆</span>
                        <span className="text-[10px]"></span>
                     </button>
                     {/* Pattern Triggers Drawer - LITERAL Exploits drawer code, only patternIds changed */}
                     {patternDrawer && (
                        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center">
                           <div className="bg-slate-900 border border-blue-700 rounded-xl shadow-2xl max-w-md w-full p-0 relative animate-in slide-in-from-bottom-10">
                              <div className="flex items-center justify-between px-6 pt-5 pb-2 border-b border-blue-700">
                                 <h2 className="text-xl font-bold text-blue-300 flex items-center gap-2"><span className="text-2xl">𝌆</span> Patterns</h2>
                                 <button className="text-slate-400 hover:text-white" onClick={() => setPatternDrawer(false)}><X size={24} /></button>
                              </div>
                              <div className="max-h-96 overflow-y-auto p-6 pt-3 space-y-3">
                                 {(() => {
                                    const patternIds = [
                                       'metrocard',
                                       'insider_trading',
                                    ...
                                    const patterns = effectsRegistry.filter(e => patternIds.includes(e.id));
                                    if (patterns.length === 0) return <div className="text-slate-400 text-center">No patterns found.</div>;
                                    return patterns.map(effect => {
                                       const isActive = activeEffects.includes(effect.id);
                                       const isReady = isEffectReady(effect.id, gameState);
                                       const charges = gameState.charges[effect.id] ?? effect.maxCharges;
                                       const rarityColors = getRarityColor(effect.rarity);
                     ...
                     <button type="button" onClick={() => discardAndDrawHand()} aria-label="Draw from deck" className="shrink-0 w-8 h-8 flex items-center justify-center bg-blue-900 hover:bg-blue-800 rounded text-blue-300 border border-blue-700 relative">
                        <img src="/icons/foundation.png" alt="Draw" className="w-8 h-8" />
